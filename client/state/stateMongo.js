...
export async function fetchMinimongo() {
    ...
    let find = db.zips.find({ zip: { $regex: _.escapeRegExp(search) } }, { limit: 10 });
    find.fetch(res => {
        stateManager.produce(draftState => {
...
export async function getTotalPop() {
    let data = await fetch(`http://localhost:3008/minimongo-api/get-total-population`);
    let json = await data.json();
    return json.total;
}
//# sourceMappingURL=stateMongo.js.map